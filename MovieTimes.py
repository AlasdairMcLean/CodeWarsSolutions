...
def movie_times(open, close, length):
    # For this example, work in minutes to avoid floating point error associated with decimals
    if close < open:
        close += 24 #adjust in case the movie theatre is open overnight
    open *= 60 #convert to minutes
    close *= 60
    total_showtimes = int(((15+ close-open)// (((length)+15))))
    showtimes=[(open // 60, 0)] #Convert back open time (which is in minutes) back into hours
    for i in range (1,total_showtimes): # Iterate through every showtime
        showtimes.append(time_tuple(open + (i * ((length) + 15))))
    return showtimes


def time_tuple(time): #Convert from decimal to hours, minutes tuple
    hours = time // 60
    mins = (time - (hours*60))
    if hours >=24:
        hours -= 24
